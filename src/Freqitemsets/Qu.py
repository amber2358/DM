...
# 2: 'yuanqu' 
Qu_data = loader.body_extractor("yuanqu")
print(f"共加载 {len(Qu_data)} 行曲，内容示意：")
print(Qu_data[0])
...
# 保存分词结果
fenci_data = []
path1 = os.path.join(project_root, '../data/Qu/fenci_qu.txt')
if not os.path.exists(path1):
    for line_idx, line in enumerate(Qu_data):
...
output_path = os.path.join(project_root, '../data/Qu/merged_frequent_itemsets.json')
merged_freq_itemsets.to_json(output_path, orient='records', force_ascii=False, indent=2)
print(f"已保存频繁项集到：{output_path}")

rules_output_path = os.path.join(project_root, '../data/Qu/association_rules.csv')
